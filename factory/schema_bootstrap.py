# -*- coding: utf-8 -*-
"""Предложить ReportSchema для НЕЗНАКОМОГО формата отчёта через LLM. ОДНОРАЗОВО.

Это единственное место в системе, где LLM трогает СХЕМУ (структуру), а не факт.
Разница принципиальна: для гипотез у нас есть цитатный гейт (проверяем дословное
вхождение в источник) — для схемы такой проверки в принципе не может быть (схема — это
КЛАССИФИКАЦИЯ словаря файла, а не факт, который где-то дословно написан). Поэтому:

  1. LLM смотрит только НА ПОДПИСИ (колонка-якорь), не на данные — предлагает схему.
  2. Схема сохраняется в файл — человек может прочитать/поправить перед боевым
     использованием (как обычный редактируемый config, а не решение модели).
  3. После сохранения ВСЁ дальнейшее чтение и рассуждение — снова 100% детерминированы
     (TailingsReader просто читает файл схемы, LLM в дальнейшем парсинге не участвует).
  4. Автоматическая подстраховка: сразу после бутстрапа гоняем TailingsReader с
     предложенной схемой и смотрим на её же валидационные warnings (баланс форм и
     т.п.) — если схема действительно неверна, это обычно видно по несходящимся
     числам, а не только «на глаз».

Запуск:
    uv run python -m factory.schema_bootstrap "Хвосты <незнакомая фабрика>.xlsx"
"""
...
from factory.schema import ElementSpec, ReportSchema, save_schema
...
def _scan_labels(path: str, anchor_col: int = 2, limit: int = 400) -> list:
    """Только подписи колонки-якоря — НИКАКИХ чисел/данных не уходит в LLM."""
