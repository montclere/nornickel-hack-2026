...
def _force_layout(node_ids, adj, W, H, iters=420):
    n = len(node_ids)
    rnd = random.Random(1)
    cx, cy = W / 2, H / 2
    pos = {}
    ...
    for _ in range(iters):
        ...
        for a, b in adj:
            ...
            disp[b][0] += ux * f; disp[b][1] += uy * f
        for nid in node_ids:
            disp[nid][0] += (cx - pos[nid][0]) * grav
            disp[nid][1] += (cy - pos[nid][1]) * grav
            dx, dy = disp[nid]
    ...
    ox = (W - (maxx - minx) * s) / 2
    oy = (H - (maxy - miny) * s) / 2
    res = {nid: [(p[0] - minx) * s + ox, (p[1] - miny) * s + oy] for nid, p in pos.items()}
    ...
    for _ in range(80):
        ...
        if not moved:
            break
    for p in res.values():
        p[0] = min(W - 46, max(46, p[0])); p[1] = min(H - 34, max(34, p[1]))
    return res
...
def _graph_svg(layered, max_label=22):
    ...
    ndmap = {nd["id"]: nd for nd in nodes}

    seen, draw = set(), []
    for e in layered["edges"]:
        if e["src"] not in ndmap or e["dst"] not in ndmap:
            continue
        ...
        seen.add(key)
        draw.append(e)
    adj = list({(e["src"], e["dst"]) for e in draw})
    pos = _force_layout(ids, adj, W, H)
    ...
    for e in draw:
        ...
        edges_svg += (f'<line class="kedge" data-a="{_esc(e["src"])}" data-b="{_esc(e["dst"])}" '
                      f'x1="{x1:.1f}" y1="{y1:.1f}" x2="{x2:.1f}" y2="{y2:.1f}" '
                      f'stroke="{color}" stroke-width="1.7" opacity="0.5" '
    ...
    for nid in ids:
        ...
        r = 5 + 7 * (nd["deg"] / max_deg)
        hub = nd["deg"] >= max_deg * 0.6 and nd["deg"] >= 3
        fill, stroke = ("#1f7ae0", "#1560b8") if hub else ("#7cc3f2", "#4d9fe0")
        nodes_svg += (
            f'<g class="knode" data-id="{_esc(nid)}" transform="translate({x:.1f},{y:.1f})">'
            f'<circle class="kdot" r="{r:.1f}" fill="{fill}" stroke="{stroke}" stroke-width="1.5"/>'
            f'<text class="klabel" x="0" y="{r + 12:.0f}" text-anchor="middle" font-size="10.5" '
            f'fill="#12303f">{_esc(short)}</text>'
            f'<title>{_esc(label)} · связей: {nd["deg"]}</title></g>')
    ...
    return (f'<svg class="kgraph" viewBox="0 0 {W} {H}" width="100%" '
            f'xmlns="http://www.w3.org/2000/svg"><defs>{defs}</defs>'
            f'<g class="kedges">{edges_svg}</g><g class="knodes">{nodes_svg}</g></svg>')
...
.kgraph .kdot{{transition:filter .1s}} .kgraph .knode:hover .kdot{{filter:drop-shadow(0 2px 5px rgba(31,122,224,.4))}}
.kgraph .knode:hover .klabel{{font-weight:700}}
.kgraph .klabel{{paint-order:stroke;stroke:#fff;stroke-width:3px}}
