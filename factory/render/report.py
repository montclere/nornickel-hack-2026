...
def _card(h, runinfo=None):
    ...
    <article class="card">
      <div class="chead">
        ...
        {'<div class="axb-badge">A×B · схема</div>' if getattr(h, "bridge", None) else ''}
      ...
      {_bridge_html(getattr(h, "bridge", None), h.intervention)}
      ...
      <div class="wp"><b>Похожие практики (веб-поиск, не проверенное внедрение):</b>{wp}</div>
      {_dossier_html(getattr(h, "dossier", None), runinfo.get("dossier"))}
    </article>"""

def _bridge_html(bridge, intervention=""):
    if not bridge:
        return ""
    items = ""
    for b in bridge:
        loc = _esc(b.get("locator") or b.get("source") or "")
        q = _esc(b.get("quote") or "")
        items += (f'<div class="bru"><b>{_esc(b.get("unit", ""))}</b>'
                  + (f'<div class="q">«{q}»</div>' if q else "")
                  + (f'<span class="loc">{loc}</span>' if loc else "") + "</div>")
    return (f'<div class="bridge"><b>Привязка к схеме фабрики</b> '
            f'<span class="llm-mark">A×B</span>'
            f'<div class="small">«{_esc(intervention)}» на узлах этой фабрики:</div>{items}</div>')
...
def panels_html(layered, analysis):
    ...
    analyses = analysis if isinstance(analysis, (list, tuple)) else [analysis]
    return graph_panel + "".join(_analysis_html(a) for a in analyses if a)
...
.axb-badge{{margin-left:12px;font-size:11.5px;font-weight:800;color:#0a7f79;background:var(--teal-soft);
  border:1px solid #9fe3dd;padding:5px 12px;border-radius:20px}}
...
.bridge{{font-size:12px;margin:12px 0;padding:11px 14px;border:1px solid #9fe3dd;border-radius:10px;
  background:linear-gradient(180deg,#eefaf8,#fff)}} .bridge>b{{font-size:11px;text-transform:uppercase;color:#0a7f79}}
.bru{{margin:7px 0 0}} .bru>b{{color:#0a7f79;font-size:13px}} .bru .q{{margin:3px 0}}
.bru .loc{{display:block;color:#9db3bf;font-size:11px;margin-top:2px}}
