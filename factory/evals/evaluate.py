...
def golden_lines(path: str):
    ...
    return [ln.strip(" .0123456789") for ln in text.splitlines()
            if len(ln.strip()) > 8 and "мозгов" not in ln.lower()]

def main():
    if len(sys.argv) < 3:
        print("usage: python -m factory.evals.evaluate <отчёт.xlsx> <Гипотезы.docx> [\"KPI\"]"); return
