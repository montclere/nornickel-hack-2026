# -*- coding: utf-8 -*-
"""Глоссарий-объяснялка: КАК читать граф, кривую раскрытия, разрывы Свонсона и метрики.

Отдельная HTML-страница (ссылка из отчёта). Не просто определения — моковые рисунки
(те же виды, что в отчёте) с пояснениями и разбором метрик на конкретных числах.
Явно разделяем: ЧТО факт из данных, а ЧТО — наша эвристическая свёртка (без подтасовки).
"""
from __future__ import annotations

# ── моковые SVG-рисунки с аннотациями (иллюстративные числа) ──

def _mock_graph_svg() -> str:
    """Профиль-граф Элемент→Класс→Форма: как читать слои, ширину ребра, цвет."""
    return """
...
def _mock_liberation_svg() -> str:
    """Кривая раскрытия с отмеченным «обрывом раскрытия»."""
    W, H, pad = 620, 210, 34
    xs = [pad + (W - 2*pad) * i / 4 for i in range(5)]
    labels = ["+125", "-125+71", "-71+45", "-45+20", "-10"]
    locked = [78, 70, 55, 35, 12]       # % закрытого — падает с крупностью
    recover = [40, 52, 68, 80, 88]      # % извлекаемого — растёт
    ...
    labs = "".join(f'<text x="{xs[i]:.0f}" y="{H-10}" text-anchor="middle" font-size="10" fill="#5f7d8c">{l}</text>'
                   for i, l in enumerate(labels))
    # обрыв раскрытия — там где кривые пересекаются (закрытого стало меньше раскрытого) ~ -71+45
    xb = xs[2]
...
def _mock_swanson_svg() -> str:
    """Разрыв Свонсона (LBD): A→B, B→C есть, прямого A→C нет → скрытая связь."""
    return """
...
# метрики: (термин, диапазон, формула, как читать, числовой пример)
METRICS = [
