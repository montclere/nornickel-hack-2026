...
class BuildKnowledgeBase:
    ...
    def _chunk(self, doc: Document, text: str) -> list[Chunk]:
        """Нарезка документа на фрагменты.

        TODO: реальный чанкер (по абзацам/окнам с перекрытием).
        Пока скелет — один фрагмент на документ.
        """
        if not text:
            return []
        return [Chunk(id=f"{doc.id}::0", doc_id=doc.id, text=text, position=0)]
