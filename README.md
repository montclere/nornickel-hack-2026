# «Феникс» — фабрика научно-исследовательских гипотез
...
провальные — часто сканы/PDF). Выход: ранжированный список карточек-гипотез
...
scripts/                    build_corpus.py — сборка корпуса и сканов
frontend/                   SPA: React + Cytoscape + Tailwind
...
генераторы гипотез с анти-граблями, скоринг и дообучаемый ранкер, сборка карточек с
проверкой «никаких новых сущностей», HTTP API с SQLite-персистом и фронтенд. Полный
сценарий (build_graph → generate → feedback → chat) проходит на заглушках за секунды.

Дальше — настоящие адаптеры поверх собранного корпуса: OCR сканов, извлечение фактов и
сборка графа через LLM, агент-разведчик и скрипт «переоткрытия» (обрезать корпус по году
и показать, что топ-гипотеза подтверждается более поздней публикацией).
